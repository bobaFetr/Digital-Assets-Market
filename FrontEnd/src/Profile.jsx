...
const formatBalanceValue = value => Number(value || 0).toLocaleString(undefined, {
...
export default function Profile() {
  ...
  const loadAccountSummary = async () => {
    ...
    try {
      const [accountExport, bankAccounts] = await Promise.all([request("/api/users/me/export", {
        headers: {
      ...
      <main>
        ...
        <div className="section-grid">
          <SectionCard title="Profile Overview" description="Your demo account identity, profile picture, and quick account summary.">
            ...
            {!isLoading && !error ? <div className="section-grid">
                <div>
                  <div>
                    <img src={resolveTrustedImageUrl(profile?.profilePictureUrl, DEFAULT_PROFILE_PICTURE, buildUrl)} alt="Profile" onError={event => {
                  ...
                  <div className="section-grid">
                    <div className="ui-field-row">
                      ...
                      <label className="ui-button ui-button--secondary">
                        ...
                        <input className="visually-hidden" type="file" accept="image/*" onChange={handleProfilePictureChange} disabled={isUploadingPicture} />
                      </label>
                    </div>
                ...
                {!isAccountSummaryLoading && !accountSummaryError ? <div className="section-grid">
                    <div>
                      <div>Fiat balances</div>
                      <div className="stat-grid">
                        {["EUR", "USD"].map(currency => <div key={currency} className="stat-card">
                    ...
                    <div>
                      <div>Currencies used</div>
                      <div className="tag-row">
                        {usedCurrencies.length > 0 ? usedCurrencies.map(currency => <span key={currency} className="tag-pill">{currency}</span>) : <span>No currencies used yet.</span>}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="section-grid">
                        {wallets.length > 0 ? wallets.map(wallet => <div key={wallet.walletID || wallet.walletId || `${wallet.currency}-${wallet.createdAt}`}>


                            <span>{wallet.currency}</span>
                            <strong>{formatBalanceValue(wallet.balance)}</strong>
                          </div>) : <span>No wallets available.</span>}
                      </div>
                    </div>
                  </div> : null}
              </div> : null}
        ...
        <div className="section-grid section-grid--two">
          <SectionCard title="Personal Details" description="Update your username and export your account data when you need a full activity snapshot." actions={<button className="ui-button ui-button--secondary" onClick={handleDownloadAccountInfo} disabled={isDownloadingInfo}>{isDownloadingInfo ? "Preparing export..." : "Download Account Export"}</button>}>
          ...
          <SectionCard title="Fund Internal Wallet" description="Add demo money with a card. This simulates a platform deposit and updates your internal wallet balance." actions={<button className="ui-button ui-button--secondary" onClick={() => navigate("/wallets")}>Manage Wallets</button>}>
            ...
            <form onSubmit={handleAddMoney} className="ui-form-grid">
              ...
              <div className="action-row">
                ...
                {savedCardForDeposits ? <button type="button" className="ui-button ui-button--secondary" onClick={() => {
        ...
        <div className="section-grid section-grid--two">
          ...
          <SectionCard title="Danger Zone" description="Delete the account only if you really want to remove your demo access and wallet records.">
            ...
            <form onSubmit={handleDeleteAccount} className="ui-form-grid">
